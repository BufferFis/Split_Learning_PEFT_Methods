...
# Client trainer
class ClientTrainer:
    def __init__(self, client_model, tokenizer, device=None):  # Change here
        self.client_model = client_model
        self.tokenizer = tokenizer
        self.device = device if device is not None else xm.xla_device()  # Change here
    ...
    def prepare_dataset(self, csv_file, batch_size=4):
        ...
        dataloader = DataLoader(
            ...
            collate_fn=collator,
            shuffle=True,
            drop_last=True  # Add this for TPU compatibility
        )
    ...
    def train(self, train_dataloader, eval_dataloader=None, epochs=3):
        ...
        for epoch in range(epochs):
            ...
            for batch_idx, batch in enumerate(tqdm(train_dataloader, desc=f"Epoch {epoch+1}")):
                ...
                # Use step closures for better TPU performance
                def _step_fn():
                    self.optimizer.zero_grad()
                    loss.backward()
                    return loss.item()

                # Replace direct loss.backward() with:
                loss_value = xm.add_step_closure(_step_fn, run_async=False)
    ...
    def generate_from_input_ids(self, input_ids, max_new_tokens=50):
        ...
        # Add padding to ensure static shapes
        ...
        with torch.no_grad():
            for _ in range(max_new_tokens):
                ...
                # Mark TPU step after each token generation
                xm.mark_step()

                # After adding next token:
                if current_ids.shape[1] >= max_len:
                    break
