...
- Per-step JSON logging of training loss and perplexity (and validation checkpoints) at output_dir/metrics.jsonl
- NEW: Robust downloader that tries both 'main' and 'master' branches of the E2E repo
"""
...
E2E_CANDIDATE_URLS = {
    "train": [
        "https://raw.githubusercontent.com/tuetschek/e2e-dataset/main/e2e-dataset/trainset.csv",
        "https://raw.githubusercontent.com/tuetschek/e2e-dataset/master/e2e-dataset/trainset.csv",
    ],
    "dev": [
        "https://raw.githubusercontent.com/tuetschek/e2e-dataset/main/e2e-dataset/devset.csv",
        "https://raw.githubusercontent.com/tuetschek/e2e-dataset/master/e2e-dataset/devset.csv",
    ],
    "test": [
        "https://raw.githubusercontent.com/tuetschek/e2e-dataset/main/e2e-dataset/testset.csv",
        "https://raw.githubusercontent.com/tuetschek/e2e-dataset/master/e2e-dataset/testset.csv",
    ],
}
...
def download_with_fallback(urls: List[str], dest_path: str, timeout: int = 60) -> str:
    """
    Try each URL in order until one succeeds. Returns the URL that worked.
    Raises the last error if all fail.
    """
    last_err = None
    for u in urls:
        try:
            print(f"[download] Trying {u}")
            download_file(u, dest_path, timeout=timeout)
            print(f"[download] OK -> {dest_path}")
            return u
...
def load_e2e_csvs(data_dir: Optional[str], work_dir: str) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    """
    Load train/dev/test CSVs. If data_dir is None, download (try 'main' then 'master') into work_dir.
    Expected columns: 'mr' and 'ref' for train/dev. Test may or may not include 'ref'.
    """
    safe_mkdir(work_dir)
    if data_dir and os.path.isdir(data_dir):
        ...
        if not os.path.exists(train_fp):
            download_with_fallback(E2E_CANDIDATE_URLS["train"], train_fp)
        if not os.path.exists(dev_fp):
            download_with_fallback(E2E_CANDIDATE_URLS["dev"], dev_fp)
        if not os.path.exists(test_fp):
            download_with_fallback(E2E_CANDIDATE_URLS["test"], test_fp)
        train_df = pd.read_csv(train_fp)
        dev_df   = pd.read_csv(dev_fp)
        test_df  = pd.read_csv(test_fp)
...
class E2ETrainDataset(Dataset):
    def __init__(self, pairs, tokenizer, max_source_len, max_target_len, delimiter: str = " =>"):
        self.examples = pairs
        self.tok = tokenizer
...
class PadCollator:
    ...
    def __call__(self, batch):
        ...
        for x in batch:
            ...
            input_ids.append(torch.cat([x["input_ids"], torch.full((pad_len,), self.pad_token_id, dtype=torch.long)]))
...
def load_checkpoint(base_model_name: str, device: torch.device, resume_path: str):
...
def compute_cider(references: List[List[str]], hypotheses: List[str], n: int = 4, sigma: float = 6.0) -> float:
    eps = 1e-12
    ...
    def cider_for_pair(h_tokens: List[str], refs_tokens: List[List[str]]) -> float:
        ...
        gp = 1.0 if not ref_lens else math.exp(-((len(h_tokens) - np.mean(ref_lens))**2) / (2 * sigma * sigma))
...
def evaluate_with_metrics(grouped_refs: Dict[str, List[str]], mr_list: List[str], generations: List[str]) -> Dict[str, float]:
    references = [grouped_refs.get(mr, [""]) for mr in mr_list]
    ...
    if evaluate_fn is not None:
        try:
            ...
            norm = {k.upper().replace("-", "_").replace(" ", "_"): float(v) for k, v in scores.items()}
            if "CIDER" in norm and "CIDEr" not in norm:
                norm["CIDEr"] = norm["CIDER"]
            out = {
                "BLEU": norm.get("BLEU", corpus_bleu_nltk(references, hypotheses)),
                "NIST": norm.get("NIST", corpus_nist_nltk(references, hypotheses)),
                "METEOR": norm.get("METEOR", corpus_meteor_nltk(references, hypotheses)),
                "ROUGE_L": norm.get("ROUGE_L", corpus_rouge_l(references, hypotheses)),
                "CIDEr": norm.get("CIDEr", compute_cider(references, hypotheses)),
            }
            return out
        except Exception:
            pass
...
def generate_for_eval(model: nn.Module, tokenizer, mrs: List[str], device: torch.device,
                      num_beams: int = 10, max_new_tokens: int = 120,
                      delimiter: str = " =>", no_repeat_ngram_size: int = 3) -> List[str]:
    model.eval()
    generations = []
    for mr in mrs:
        ...
        generations.append(normalize_ws(text))
    model.train()
    return generations
...
def main():
    ...
    parser.add_argument("--model_name", type=str, default="gpt2", choices=["gpt2", "gpt2-medium"])
    ...
    parser.add_argument("--sample_every_steps", type=int, default=100)
    parser.add_argument("--num_beams", type=int, default=10)
    parser.add_argument("--gen_max_new_tokens", type=int, default=120)
    parser.add_argument("--no_repeat_ngram_size", type=int, default=3)
    parser.add_argument("--fp16", action="store_true")
    parser.add_argument("--metrics_jsonl", type=str, default="metrics.jsonl")
    ...
    # Load data (prefer local path if provided)
    ...
    # Dataloaders
    train_dataset = E2ETrainDataset(train_pairs, tokenizer, args.max_source_len, args.max_target_len)
    dev_dataset = E2ETrainDataset(dev_pairs, tokenizer, args.max_source_len, args.max_target_len)
    collator = PadCollator(pad_token_id=tokenizer.pad_token_id)
    train_loader = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True, num_workers=2, pin_memory=True, collate_fn=collator)
    dev_loader = DataLoader(dev_dataset, batch_size=args.batch_size, shuffle=False, num_workers=2, pin_memory=True, collate_fn=collator)

    optimizer = torch.optim.AdamW(model.parameters(), lr=args.learning_rate, weight_decay=args.weight_decay)
    total_training_steps = (len(train_loader) // max(1, args.gradient_accumulation_steps)) * args.num_epochs
    scheduler = get_linear_schedule_with_warmup(optimizer, num_warmup_steps=args.warmup_steps, num_training_steps=total_training_steps)
    ...
    metrics_path = os.path.join(args.output_dir, args.metrics_jsonl)
    safe_mkdir(args.output_dir)
    metrics_f = open(metrics_path, "a", encoding="utf-8")
    def log_jsonl(obj: Dict):
        obj = dict(obj)
        obj.setdefault("timestamp", time.time())
        metrics_f.write(json.dumps(obj, ensure_ascii=False) + "\n")
        metrics_f.flush()
        ...
        for epoch in range(start_epoch, args.num_epochs):
            for step, batch in enumerate(train_loader):
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0:
                    ...
                    lr = float(scheduler.get_last_lr()[0]) if scheduler is not None else float(args.learning_rate)
                    log_jsonl({"phase": "train", "epoch": epoch + 1, "global_step": global_step, "loss": train_loss, "ppl": train_ppl, "lr": lr})

                    if args.logging_steps > 0 and global_step % args.logging_steps == 0:
                        avg_loss = running_loss / args.logging_steps
                        avg_ppl = math.exp(min(20.0, avg_loss))
                        print(f"[epoch {epoch+1}/{args.num_epochs}] step {global_step} - loss: {avg_loss:.4f} - ppl: {avg_ppl:.2f}")
                        running_loss = 0.0
                    ...
                    if args.eval_steps > 0 and global_step % args.eval_steps == 0:
                        ...
                        log_jsonl({"phase": "val", "epoch": epoch + 1, "global_step": global_step, "loss": float(val_loss), "ppl": float(val_ppl)})
                        if val_loss + 1e-6 < best_val_loss:
                            ...
                            if patience_count >= args.patience:
                                ...
                                save_checkpoint(model, tokenizer, optimizer, scheduler, scaler, args.output_dir, global_step, epoch, best_val_loss, patience_count, base_model_name)
                                raise StopIteration
                    ...
                    if args.sample_every_steps > 0 and global_step % args.sample_every_steps == 0:
                        try:
                            ...
                            if sanity_mr:
                                model.eval()
                                hyp = generate_for_eval(model, tokenizer, [sanity_mr], device,
                                ...
                                log_jsonl({"phase": "sanity", "epoch": epoch + 1, "global_step": global_step, "mr": sanity_mr, "hyp": hyp, "ref_sample": refs[:1]})
                        except Exception as e:
                    ...
                    if args.save_steps > 0 and global_step % args.save_steps == 0:
                        ckpt_dir = save_checkpoint(model, tokenizer, optimizer, scheduler, scaler, args.output_dir, global_step, epoch, best_val_loss, patience_count, base_model_name)
                        print(f"Saved checkpoint: {ckpt_dir}")

            ckpt_dir = save_checkpoint(model, tokenizer, optimizer, scheduler, scaler, args.output_dir, global_step, epoch, best_val_loss, patience_count, base_model_name)
            print(f"Saved end-of-epoch checkpoint: {ckpt_dir}")
            log_jsonl({"phase": "epoch_end", "epoch": epoch + 1, "global_step": global_step})
    finally:
        try:
            metrics_f.close()
        except Exception:
    ...
    generations = generate_for_eval(model, tokenizer, test_mrs, device, num_beams=args.num_beams, max_new_tokens=args.gen_max_new_tokens, delimiter=" =>", no_repeat_ngram_size=args.no_repeat_ngram_size)
